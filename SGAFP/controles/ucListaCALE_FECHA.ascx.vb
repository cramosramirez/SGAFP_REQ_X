...
''' -----------------------------------------------------------------------------
''' Project	 : SGAFP
''' Class	 : ucListaCALE_FECHA
''' 
''' -----------------------------------------------------------------------------
''' <summary>
''' Clase del Control de Usuario para mostrar un listado de la tabla CALE_FECHA
''' </summary>
''' <remarks>
...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucListaCALE_FECHA
    Inherits ucListaBase
 
    Private mComponente As New cCALE_FECHA
    Public Event Seleccionado(ByVal ID_CALE As Decimal) 
    Public Event Editando(ByVal ID_CALE As Decimal)
    Public Event Eliminado(ByVal ID_CALE As Decimal)
...
#Region"Propiedades"
    ...
    Public Property VerID_CALE() As Boolean
        Get
            Return Me.dxgvLista.Columns("ID_CALE").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("ID_CALE").Visible = Value
        End Set
    End Property
    ...
    Public Property VerMINUTOS() As Boolean
        Get
            Return Me.dxgvLista.Columns("MINUTOS").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("MINUTOS").Visible = Value
        End Set
    End Property

    Public Property HeaderID_CALE() As String
        Get
            Return Me.dxgvLista.Columns("ID_CALE").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("ID_CALE").Caption = Value
        End Set
    End Property
    ...
    Public Property HeaderMINUTOS() As String
        Get
            Return Me.dxgvLista.Columns("MINUTOS").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("MINUTOS").Caption = Value
        End Set
    End Property
...
#End Region
    ...
    Public Function CargarDatosCache(ByVal nombreSesion As String) As Integer
        ...
        Me.odsCALE_FECHAcache.SelectParameters("nombreSesion_ucListaCALE_FECHA").DefaultValue = nombreSesion
        Me.odsCALE_FECHAcache.DataBind()
        Me.dxgvLista.DataSourceID = "odsCALE_FECHAcache"
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Función que Carga la información de los registros de la tabla CALE_FECHA
    ''' </summary>
    ''' <remarks>
    ''' </remarks>
    ''' <history>
    ...
    Protected Sub dxgvLista_CustomJSProperties(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewClientJSPropertiesEventArgs) Handles dxgvLista.CustomJSProperties
        If Me.PermitirSeleccionParaCombo Then
            ...
            For i As Integer = 0 To grid.VisibleRowCount - 1
                keyValues(i) = grid.GetRowValues(i, "ID_CALE")
    ...
    Protected Sub dxgvLista_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles dxgvLista.Init
        ...
        Me.dxgvLista.Columns("Edicion").Visible = Me.PermitirEditar
    ...
    Protected Sub dxgvLista_RowCommand(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewRowCommandEventArgs) Handles dxgvLista.RowCommand
        ...
        If e.CommandArgs.CommandName = "Eliminar" Then
            Dim bCaleFecha As New cCALE_FECHAcache
            bCaleFecha.Eliminar(e.KeyValue, Me.REFERENCIA)
            Me.dxgvLista.DataBind()
            RaiseEvent Eliminado(e.KeyValue)
        End If
    End Sub

    Public Function DevolverSeleccionados() As listaCALE_FECHA
        Dim mLista As New listaCALE_FECHA
        For Each llave As Decimal In Me.dxgvLista.GetSelectedFieldValues("ID_CALE")
            Dim lEntidad As New CALE_FECHA
            lEntidad.ID_CALE = llave
            mLista.Add(lEntidad)
        Next
        Return mLista
    End Function
    ...
    Protected Sub dxgvLista_CustomButtonCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomButtonCallbackEventArgs) Handles dxgvLista.CustomButtonCallback
        If e.ButtonID = "btnEliminar" Then
            Dim lEntidad As CALE_FECHA = CType(Me.dxgvLista.GetRow(e.VisibleIndex), CALE_FECHA)

            If Me.mComponente.EliminarCALE_FECHA(lEntidad.ID_CALE) <> 1 Then
    ...
    Protected Sub dxgvLista_CustomUnboundColumnData(sender As Object, e As DevExpress.Web.ASPxGridView.ASPxGridViewColumnDataEventArgs) Handles dxgvLista.CustomUnboundColumnData
        If e.Column.FieldName = "HORAS" Then
            Dim minutos As Integer = e.GetListSourceFieldValue("MINUTOS")
            Dim horas As String = ""
            If minutos < 60 Then
                horas = "0:" + Format(minutos, "0#")
            Else
                horas = Format(Math.Truncate(minutos / 60), "0") + ":" + Format(minutos Mod 60, "0#")
            End If
            e.Value = horas
        End If
    End Sub
