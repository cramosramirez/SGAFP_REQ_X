...
''' -----------------------------------------------------------------------------
''' Project	 : SGAFP
''' Class	 : ucListaACCION_PARTI_SINDOCTOS
''' 
''' -----------------------------------------------------------------------------
''' <summary>
''' Clase del Control de Usuario para mostrar un listado de la tabla ACCION_PARTI_SINDOCTOS
''' </summary>
''' <remarks>
...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucListaACCION_PARTI_SINDOCTOS
    Inherits ucListaBase
 
    Private mComponente As New cACCION_PARTI_SINDOCTOS
    Public Event Seleccionado(ByVal ID_ACCION_PARTI As Decimal) 
    Public Event Editando(ByVal ID_ACCION_PARTI As Decimal) 
...
#Region"Propiedades"
    ...
    Public Property VerID_ACCION_PARTI() As Boolean
        Get
            Return Me.dxgvLista.Columns("ID_ACCION_PARTI").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("ID_ACCION_PARTI").Visible = Value
        End Set
    End Property
    ...
    Public Property VerCANT_PARTICIPANTES() As Boolean
        Get
            Return Me.dxgvLista.Columns("CANT_PARTICIPANTES").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("CANT_PARTICIPANTES").Visible = Value
        End Set
    End Property

    Public Property VerES_ACTIVA() As Boolean
        Get
            Return Me.dxgvLista.Columns("ES_ACTIVA").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("ES_ACTIVA").Visible = Value
        End Set
    End Property
    ...
    Public Property HeaderID_ACCION_PARTI() As String
        Get
            Return Me.dxgvLista.Columns("ID_ACCION_PARTI").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("ID_ACCION_PARTI").Caption = Value
        End Set
    End Property
    ...
    Public Property HeaderCANT_PARTICIPANTES() As String
        Get
            Return Me.dxgvLista.Columns("CANT_PARTICIPANTES").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("CANT_PARTICIPANTES").Caption = Value
        End Set
    End Property

    Public Property HeaderES_ACTIVA() As String
        Get
            Return Me.dxgvLista.Columns("ES_ACTIVA").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("ES_ACTIVA").Caption = Value
        End Set
    End Property
...
#End Region
    ...
    Protected Sub dxgvLista_CustomJSProperties(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewClientJSPropertiesEventArgs) Handles dxgvLista.CustomJSProperties
        If Me.PermitirSeleccionParaCombo Then
            ...
            For i As Integer = 0 To grid.VisibleRowCount - 1
                keyValues(i) = grid.GetRowValues(i, "ID_ACCION_PARTI")
    ...
    Public Function DevolverSeleccionados() As listaACCION_PARTI_SINDOCTOS
        Dim mLista As New listaACCION_PARTI_SINDOCTOS
        For Each llave As Decimal In Me.dxgvLista.GetSelectedFieldValues("ID_ACCION_PARTI")
            Dim lEntidad As New ACCION_PARTI_SINDOCTOS
            lEntidad.ID_ACCION_PARTI = llave
            mLista.Add(lEntidad)
        Next
        Return mLista
    End Function
    ...
    Protected Sub dxgvLista_CustomButtonCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomButtonCallbackEventArgs) Handles dxgvLista.CustomButtonCallback
        If e.ButtonID = "btnEliminar" Then
            Dim lEntidad As ACCION_PARTI_SINDOCTOS = CType(Me.dxgvLista.GetRow(e.VisibleIndex), ACCION_PARTI_SINDOCTOS)

            If Me.mComponente.EliminarACCION_PARTI_SINDOCTOS(lEntidad.ID_ACCION_PARTI) <> 1 Then
    ...
    Protected Sub dxgvLista_CustomUnboundColumnData(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewColumnDataEventArgs) Handles dxgvLista.CustomUnboundColumnData
        ...
        ElseIf e.Column.FieldName = "AUTORIZACION" Then
            If e.GetListSourceFieldValue("TIPO_AUTORIZACION") = "S/D" Then
                e.Value = "SIN DUI/NIT"
            ElseIf e.GetListSourceFieldValue("TIPO_AUTORIZACION") = "M/E" Then
                e.Value = "MENORES DE 16 AÑOS"
            End If
        End If
    End Sub
End Class
