...
''' -----------------------------------------------------------------------------
''' Project	 : SGAFP
''' Class	 : ucVistaDetalleDEPARTAMENTO
''' 
''' -----------------------------------------------------------------------------
''' <summary>
...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucVistaDetalleDEPARTAMENTO
    Inherits ucBase
...
#Region"Propiedades"
    ...
    Public Property CODIGO_DEPARTAMENTO() As String
        Get
            Return Me.txtCODIGO_DEPARTAMENTO.Text
        End Get
        Set(ByVal Value As String)
            Me._CODIGO_DEPARTAMENTO = Value
            Me.txtCODIGO_DEPARTAMENTO.Text = Value
            If Me._CODIGO_DEPARTAMENTO <> "" Then
                Me.CargarDatos()
            Else
                Me.LimpiarControles()
                Me.Nuevo()
    ...
    Private mEntidad As DEPARTAMENTO
...
#End Region
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Función que Carga la información del registro de la tabla DEPARTAMENTO
    ''' </summary>
    ''' <remarks>
    ''' </remarks>
    ''' <history>
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Función que Guarda la información del registro en la tabla DEPARTAMENTO
    ''' </summary>
    ''' <returns>
    ''' ""                              : Si no existe error
    ...
    Public Function Actualizar() As String
        ...
        mEntidad = New DEPARTAMENTO
        If Me.txtCODIGO_DEPARTAMENTO.Text = "0" Then
            Me.AsignarMensaje("Asigne un valor al Codigo departamento que sea valido", True, True)
            Return "Error"
        End If
        ...
        If Me._nuevo Then
            If mComponente.AgregarDEPARTAMENTO(mEntidad) <> 1 Then
                Me.AsignarMensaje("Error al Guardar registro.", True, True)
                Return "Error al Guardar registro."
            End If
         Else
            If mComponente.ActualizarDEPARTAMENTO(mEntidad) <> 1 Then
                Me.AsignarMensaje("Error al Guardar registro.", True, True)
                Return "Error al Guardar registro."
            End If
        End If
        Me.txtCODIGO_DEPARTAMENTO.Text = mEntidad.CODIGO_DEPARTAMENTO.ToString()
        Me._nuevo = False
        If Not Me.ViewState("nuevo") Is Nothing Then Me.ViewState.Remove("nuevo")
        Me.ViewState.Add("nuevo", Me._nuevo)
        Return ""
