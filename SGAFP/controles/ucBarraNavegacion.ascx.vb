...
Partial Class controles_ucBarraNavegacion
    ...
    Private Const strImagenesNuevo As String = "~/imagenes/Nuevo.png"
    ...
    Private Const strImagenCancelar As String = "~/imagenes/Cancelar.gif"
    Private Const strImagenEditar As String = "~/imagenes/Editar.png"
    ...
    Private Const strImagenGuardar As String = "~/imagenes/ButtonSave.png"
    Private Const strTextoGuardar As String = "Guardar"
    Private Const strNombreMenuGuardar As String = "miGuardar"
    Private Const strImagenExcel As String = "~/imagenes/excel.png"
...
#Region "Exportación Grids DevExpress"
...
#Region "Opciones Estáticas"
    ...
    Public Sub HabilitarEdicion(ByVal edicion As Boolean)
        ...
        Dim lMenuEditarCancelar As DevExpress.Web.ASPxMenu.MenuItem = mDinamico.Items.FindByName(strNombreMenuEditar)
        If edicion Then
            lMenuEditarCancelar.Image.Url = strImagenCancelar
            lMenuEditarCancelar.Text = strTextoCancelar
        Else
            lMenuEditarCancelar.Image.Url = strImagenEditar
...
#Region "Opciones Dinámicas"
    ...
    Public Sub VerOpcion(ByVal CommandName As String, ByVal EsVisible As Boolean)
        For Each lMenu As DevExpress.Web.ASPxMenu.MenuItem In mDinamico.Items
    ...
    Private Sub CrearMenuAgregar()
        Dim lMenuAgregar As New DevExpress.Web.ASPxMenu.MenuItem
        lMenuAgregar.Image.Url = strImagenesNuevo
    ...
    Private Sub CrearMenuEditar()
        Dim lMenuEditar As New DevExpress.Web.ASPxMenu.MenuItem
        If _HabilitarEdicion Then
            lMenuEditar.Image.Url = strImagenCancelar
            lMenuEditar.Text = strTextoCancelar
        Else
            lMenuEditar.Image.Url = strImagenEditar
    ...
    Private Sub CrearMenuGuardar()
        Dim lMenuGuardar As New DevExpress.Web.ASPxMenu.MenuItem
        lMenuGuardar.Image.Url = strImagenGuardar
    ...
    Private Sub CrearMenuExportarAExcel()
        Dim lMenuExportarAExcel As New DevExpress.Web.ASPxMenu.MenuItem
    ...
    Private Sub CrearMenuExportarAPDF()
        Dim lMenuExportarAPDF As New DevExpress.Web.ASPxMenu.MenuItem
    ...
    Public Sub CargarOpciones(ByVal aLista As ListaOpcionesBarra)
        ...
        For Each lOpcion As OpcionesBarra In aLista
            Dim lMenu As New DevExpress.Web.ASPxMenu.MenuItem
            If i = 0 Then
            ...
            If lOpcion.CausesValidation AndAlso lOpcion.AtributoKey.ToLower <> "onclick" Then
                If HabilitarCallBacks Then
                ...
                Else
                    ...
                    mDinamico.ClientSideEvents.ItemClick = String.Format("function(s, e) {{ {0} }}", ClientScript)
                End If
            ElseIf lOpcion.CausesValidation AndAlso lOpcion.AtributoKey.ToLower = "onclick" Then
                If HabilitarCallBacks Then
                ...
                Else
                    ...
                    mDinamico.ClientSideEvents.ItemClick = String.Format("function(s, e) {{ {0} }}", ClientScript)
                End If
            ElseIf lOpcion.AtributoKey <> "" AndAlso lOpcion.AtributoKey.ToLower = "onclick" Then
                If HabilitarCallBacks Then
...
#End Region
    ...
    Protected Sub mDinamico_ItemClick(ByVal source As Object, ByVal e As DevExpress.Web.ASPxMenu.MenuItemEventArgs) Handles mDinamico.ItemClick
