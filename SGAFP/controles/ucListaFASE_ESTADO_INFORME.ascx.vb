...
''' -----------------------------------------------------------------------------
''' Project	 : SGAFP
''' Class	 : ucListaFASE_ESTADO_INFORME
''' 
''' -----------------------------------------------------------------------------
''' <summary>
''' Clase del Control de Usuario para mostrar un listado de la tabla FASE_ESTADO_INFORME
''' </summary>
''' <remarks>
...
''' </history>
''' -----------------------------------------------------------------------------
Partial Class controles_ucListaFASE_ESTADO_INFORME
    Inherits ucListaBase
 
    Private mComponente As New cFASE_ESTADO_INFORME
    Public Event Seleccionado(ByVal ID_FASE_ESTADO As Decimal) 
    Public Event Editando(ByVal ID_FASE_ESTADO As Decimal) 
...
#Region"Propiedades"
    ...
    Public Property VerNOMBRE_FASE_ESTADO() As Boolean
        Get
            Return Me.dxgvLista.Columns("NOMBRE_FASE_ESTADO").Visible
        End Get
        Set(ByVal Value As Boolean)
            Me.dxgvLista.Columns("NOMBRE_FASE_ESTADO").Visible = Value
        End Set
    End Property
    ...
    Public Property HeaderNOMBRE_FASE_ESTADO() As String
        Get
            Return Me.dxgvLista.Columns("NOMBRE_FASE_ESTADO").Caption
        End Get
        Set(ByVal Value As String)
            Me.dxgvLista.Columns("NOMBRE_FASE_ESTADO").Caption = Value
        End Set
    End Property
...
#End Region
    ...
    Protected Sub dxgvLista_CustomJSProperties(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewClientJSPropertiesEventArgs) Handles dxgvLista.CustomJSProperties
        If Me.PermitirSeleccionParaCombo Then
            ...
            For i As Integer = 0 To grid.VisibleRowCount - 1
                keyValues(i) = grid.GetRowValues(i, "ID_FASE_ESTADO")
                keyNames(i) = grid.GetRowValues(i, "NOMBRE_FASE_ESTADO")
            Next i
    ...
    Public Function DevolverSeleccionados() As listaFASE_ESTADO_INFORME
        Dim mLista As New listaFASE_ESTADO_INFORME
        For Each llave As Decimal In Me.dxgvLista.GetSelectedFieldValues("ID_FASE_ESTADO")
            Dim lEntidad As New FASE_ESTADO_INFORME
            lEntidad.ID_FASE_ESTADO = llave
            mLista.Add(lEntidad)
        Next
        Return mLista
    End Function
    ...
    Protected Sub dxgvLista_CustomButtonCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomButtonCallbackEventArgs) Handles dxgvLista.CustomButtonCallback
        If e.ButtonID = "btnEliminar" Then
            Dim lEntidad As FASE_ESTADO_INFORME = CType(Me.dxgvLista.GetRow(e.VisibleIndex), FASE_ESTADO_INFORME)

            If Me.mComponente.EliminarFASE_ESTADO_INFORME(lEntidad.ID_FASE_ESTADO) <> 1 Then
