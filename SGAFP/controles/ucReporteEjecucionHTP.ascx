<%@ Control Language="VB" AutoEventWireup="false" CodeFile="ucReporteEjecucionHTP.ascx.vb" Inherits="controles_ucReporteEjecucionHTP" %>
...
<script language="javascript" type="text/javascript">    
    ...
    function VerReporte() {
        ...
        var ID_SITIO_CAPACITACION = '-1';
        var EJERCICIO = '';
        var OPCION = '';        
        if (cbxProveedor.GetValue() != null) { ID_PROVEEDOR_AF = cbxProveedor.GetValue(); }
        if (cbxCentroFormacion.GetValue() != null) { ID_SITIO_CAPACITACION = cbxCentroFormacion.GetValue(); }
        if (cbxEjercicio.GetValue() != null) { EJERCICIO = cbxEjercicio.GetValue(); }

        if (ID_PROVEEDOR_AF == '-1') {
            OPCION = 'CargarInformacion';
        }
        else if (ID_SITIO_CAPACITACION == '-1'){
            OPCION = 'CargarInformacionProveedor';
        }
        else {
            OPCION = 'CargarInformacionCentro';
        }
        cReporte.PerformCallback(OPCION + ';' + ID_PROVEEDOR_AF + ';' + ID_SITIO_CAPACITACION + ';' + EJERCICIO);
        LoadingPanel.Show();
    }
</script>
     ...
     <PanelCollection>        
            <dx:PanelContent ID="PanelContent1" runat="server">   
                ...
                <uc1:ucCriteriosConsultaEjecucion ID="ucCriteriosConsultaEjecucion1" runat="server"  
                     VerPrograma="false" VerBotonMostrar="false" VerAreaFormacion="false" VerClasificacionCursos="false" VerCodigoGrupo="false"  />     
